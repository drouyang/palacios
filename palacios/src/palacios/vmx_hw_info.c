			    ...
			    uint32_t                true_msr) 
{
    ...
    uint32_t true_0 = 0;  /* Bit is 1 => MB1 */
    uint32_t true_1 = 0;  /* Bit is 0 => MBZ */
...
static int 
get_ctrl_caps(struct vmx_ctrl_field * field, 
	      uint32_t                msr)
{
...
static int 
get_cr_fields(struct vmx_cr_field * field, 
	      uint32_t              fixed_1_msr, 
	      uint32_t              fixed_0_msr) 
{
...
int 
v3_init_vmx_hw(struct vmx_hw_info * hw_info) 
{
    ...
    /* 
     * Check whether Secondary Controls are available 
     * 
	...
	/*
	 * Check For Nested Paging (EPT) Support 
	 */
