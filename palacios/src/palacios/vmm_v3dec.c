...
int 
v3_disasm(struct v3_core_info * core, 
	  void                * instr_ptr, 
	  addr_t              * rip, 
	  int                   mark) 
{
...
int 
v3_encode(struct v3_core_info * core, 
...
int 
v3_decode(struct v3_core_info * core, 
	  addr_t                instr_ptr, 
	  struct x86_instr    * instr) 
{
...
static int 
parse_operands(struct v3_core_info * core, 
	       uint8_t             * instr_ptr, 
	       struct x86_instr    * instr,
	       op_form_t             form) 
{
	...
	case MOV_MEM2AX: {
		...
		PrintError("illegal address width for %s (width=%d)\n",
...
static v3_op_type_t 
op_form_to_type(op_form_t form) 
{ 
    switch (form) {
	case LMSW:
	    return V3_OP_LMSW;
