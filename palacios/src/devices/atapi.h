 ...
 */
static int 
atapi_update_req_len(struct ide_internal * ide, 
...
static void 
atapi_setup_cmd_resp(struct ide_internal * ide, 
		     struct ide_channel  * channel, 
		     uint_t                xfer_len)
{
    struct ide_drive * drive = get_selected_drive(channel);
...
static void 
atapi_cmd_error(struct ide_internal  * ide, 
		struct ide_channel   * channel, 
		atapi_sense_key_t      sense_key, 
		atapi_add_sense_code_t asc)
{
    struct ide_drive * drive = get_selected_drive(channel);
...
static void 
atapi_cmd_nop(struct ide_internal * ide,
	      struct ide_channel  * channel) 
{
    struct ide_drive * drive = get_selected_drive(channel);
...
static int 
atapi_read_chunk(struct ide_internal * ide,
		 struct ide_channel  * channel)
{
    struct ide_drive * drive = get_selected_drive(channel);
...
static int 
atapi_update_data_buf(struct ide_internal * ide,
		      struct ide_channel  * channel) 
{
    struct ide_drive * drive = get_selected_drive(channel);    
...
static int 
atapi_read10(struct v3_core_info * core, 
...
static void 
atapi_req_sense(struct ide_internal * ide, 
		struct ide_channel  * channel) 
{
    struct ide_drive * drive = get_selected_drive(channel);
...
static int
atapi_get_capacity(struct ide_internal * ide, 
		   struct ide_channel  * channel)
{
    struct ide_drive              * drive    = get_selected_drive(channel);
...
static int 
atapi_get_config(struct ide_internal * ide,
		 struct ide_channel  * channel) 
{
    struct ide_drive         * drive = get_selected_drive(channel);
...
static int
atapi_read_toc(struct ide_internal * ide,
	       struct ide_channel * channel)
{
    struct ide_drive         * drive = get_selected_drive(channel);
...
static int 
atapi_mode_sense_cur_values(struct ide_internal         * ide,
			    struct ide_channel          * channel, 
...
static int 
atapi_mode_sense(struct ide_internal * ide, 
		 struct ide_channel  * channel) 
{
    struct ide_drive            * drive     = get_selected_drive(channel);
...
static int 
atapi_inquiry(struct ide_internal * ide, 
	      struct ide_channel  * channel) 
{
    struct ide_drive          * drive       = get_selected_drive(channel);
...
static int 
atapi_mech_status(struct ide_internal * ide,
		  struct ide_channel  * channel)
{
    struct ide_drive              * drive      = get_selected_drive(channel);
...
static int 
atapi_handle_packet(struct v3_core_info * core, 
		    struct ide_internal * ide, 
		    struct ide_channel  * channel) {
    struct ide_drive * drive = get_selected_drive(channel);
...
static void 
atapi_identify_device(struct ide_drive * drive) 
{
