...
static void 
ata_identify_device(struct ide_drive * drive) 
{
    ...
    drive_id->dword_io          = 1;     /* 32 bit accesses   */
    drive_id->dma_enable        = 1;     /* enable DMA access */
    drive_id->lba_enable        = 1;     /* enable LBA access */

    drive_id->lba_capacity      = drive->ops->get_capacity(drive->private_data) / HD_SECTOR_SIZE; /* Drive Capacity (28 bit LBA) */
    drive_id->lba_capacity_2    = drive->ops->get_capacity(drive->private_data) / HD_SECTOR_SIZE; /* Drive Capacity (48 bit LBA) */
    drive_id->rw_multiples      = 0x8000 | MAX_MULT_SECTORS;                                      /* lower byte is the maximum multiple sector size... */
...
static int
ata_read(struct ide_internal * ide, 
...
static int 
ata_write(struct ide_internal * ide,
	  struct ide_channel  * channel, 
	  uint8_t             * src, 
	  uint_t                sect_cnt)
{
...
static int 
ata_get_lba(struct ide_internal * ide,
	    struct ide_channel  * channel, 
	    uint64_t            * lba)
{
    struct ide_drive * drive    = get_selected_drive(channel);
    ...
    if ( (lba_addr.addr + sect_cnt) > (drive->ops->get_capacity(drive->private_data) / HD_SECTOR_SIZE) ) {
...
// 28 bit LBA
static int 
ata_read_sectors(struct ide_internal * ide,  
		 struct ide_channel  * channel) 
{
    struct ide_drive * drive    = get_selected_drive(channel);
    // The if the sector count == 0 then read 256 sectors (cast up to handle that value)
...
// 48 bit LBA
static int 
ata_read_sectors_ext(struct ide_internal * ide, 
		     struct ide_channel  * channel)
{
    //struct ide_drive * drive = get_selected_drive(channel);
    // The if the sector count == 0 then read 256 sectors (cast up to handle that value)
