...
{
    struct mem_stream *m = palacios_kmalloc(sizeof(struct mem_stream), GFP_KERNEL);
					 ...
					 v3_keyed_stream_open_t ot)
{
	...
	case V3_KS_WR_ONLY_CREATE: {
		...
		mykey = palacios_kmalloc(strlen(url+4)+1, GFP_KERNEL);
		...
		mks = (struct mem_keyed_stream *) palacios_kmalloc(sizeof(struct mem_keyed_stream), GFP_KERNEL);

		if (!mks) { 
		    palacios_kfree(mykey);
					  ...
					  char *key)
{
    ...
    if (!m) { 
	char *mykey = palacios_kmalloc(strlen(key)+1, GFP_KERNEL);
				     ...
				     uint64_t size)
{
	...
	mykey=palacios_kmalloc(strlen(key)+1, GFP_KERNEL);
					  ...
					  v3_keyed_stream_open_t ot)
{
    ...
    fks = palacios_kmalloc(sizeof(struct file_keyed_stream), GFP_KERNEL);
    ...
    fks->path = (char*)palacios_kmalloc(strlen(url+5)+1, GFP_KERNEL);
					   ...
					   char *key)
{
    ...
    path = (char *) palacios_kmalloc(strlen(fks->path)+strlen(key)+2, GFP_KERNEL);
    if (!path) {				
    ...
    fs = (struct file_stream *) palacios_kmalloc(sizeof(struct file_stream *), GFP_KERNEL);
...
static int resize_op(struct palacios_user_keyed_stream_op **op, uint64_t buf_len)
{
    ...
    if (!old) {
	new = palacios_kmalloc(sizeof(struct palacios_user_keyed_stream_op)+buf_len, GFP_KERNEL);
	if (!new) { 
	    return -1;
	} else {
...
int keyed_stream_connect_user(struct v3_guest *guest, unsigned int cmd, unsigned long arg, void *priv_data)
{
    ...
    url = palacios_kmalloc(len, GFP_KERNEL);
    ...
    // Create connection
    s = palacios_kmalloc(sizeof(struct user_keyed_stream), GFP_KERNEL);
...
static struct net_stream * create_net_stream(void) 
{
    ...
    ns = palacios_kmalloc(sizeof(struct net_stream), GFP_KERNEL);
...
static struct v3_keyed_stream_t * open_stream_net(char * url,v3_keyed_stream_open_t ot)
{
    ...
    nks = palacios_kmalloc(sizeof(struct net_keyed_stream), GFP_KERNEL); 
...
static int init_keyed_streams( void )
{
    ...
    user_streams = palacios_kmalloc(sizeof(struct user_keyed_streams), GFP_KERNEL);
