...
static void free_inject_data (void) {
    ...
    for(i = 0; i < MAX_INJ; i++) {
        if (top_map[i]) {
            palacios_kfree(top_map[i]->elf_data);
            palacios_kfree(top_map[i]);
        }
    }
}
...
static int vm_tophalf_inject (struct v3_guest * guest, unsigned int cmd, unsigned long arg, void * priv_data) {
    ...
    top = palacios_kmalloc(sizeof(struct top_half_data), GFP_KERNEL);
    if (IS_ERR(top)) {
    ...
    top->elf_data = palacios_kmalloc(top->elf_size, GFP_KERNEL);
